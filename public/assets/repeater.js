...
$('body').on('click', '.addPanorama', function(e){ // добавить панораму
    ...
    floorIndex = $('.panorama-wrap').last().find('.x').attr('name');
...
$('body').on('change', '.plan_input', function(e){ // добавление полей для панорамы после загрузки плана
    var addPanoramaVal = `
        <div class="form-group panorama-wrap">
            <label class="control-label panorama_label">Панорама</label>
            <input required type="file" class="form-control panorama_input" name="floor_panorama[` + floorIndex + `][]">
...
$('body').on('click', '.plan_label', function() { // открытие модального окна при клике на label
    var _this = $(this);

    if(_this.data('plan')) {
        $('.planClass, .planHotspot').remove();
        $('.modal .plan').append(`
            <img class="planClass" style="border: 1px dotted black;" src="/storage/` + _this.data('plan') + `">
        `);
