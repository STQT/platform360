  ...
  <action name="makescreenshot_init" type="Javascript" autorun="onstart"><![CDATA[
    ...
    <scene name="scene1" title="scene1" onstart="" thumburl="/storage/panoramas/unpacked/{{ $location->folderName($index) }}/thumb.jpg" lat="41.311108"  lng="69.279711" heading="">
        ...
        <action name="calc_pos_from_hfov_yaw_pitch_roll">
            ...
            @if ($location->videos)
              @foreach ($location->videos as $i => $video)
                   @php
                       $playType = 'false';
                       if ($video->play_type == App\Video::PLAY_TYPE_AUTOMATICALLY) {
                           $playType = 'false';
                       } elseif ($video->play_type == App\Video::PLAY_TYPE_MANUALLY) {
                           $playType = 'true';
                       }
                   @endphp
                   <hotspot name="video{{ $i }}"
